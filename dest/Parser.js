...
let numberLit = RegExpCombinator(/(?:+|-)*(?:[0-9]*\.[0-9]+|[0-9])+/, function (str, st) { return str; }, function (st) { return "Failed to parse a number starting at position " + st.processed.length; });
let functionLit = RegExpCombinator(/log|ln|sqrt|exp|(?:a(?:rc)?)?(?:sin|cos|tan|sec|csc|cot)/i, function (str, st) {
...
let variableLit = RegExpCombinator(/[a-zA-Z]\w*/, function (str, st) { return str; }, function (st) { return "Failed to parse a variable name starting at position " + st.processed.length; });
